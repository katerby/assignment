...
- Most text inputs enforce a maximum character count and numeric inputs enforce a maximum value, with the limit reported inline when it is exceeded. The constraint appears to be applied broadly across the application rather than field by field. In list and table views, values too long for the column are truncated for display, so a rendered cell is not always the full stored value and assertions on table text need to allow for that.
