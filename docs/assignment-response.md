...
- The application was checked in Chrome, Safari and Firefox to confirm the pages are functional and the UI behaves as expected across browsers. It was also viewed at desktop, tablet and mobile widths to confirm the layout stays usable and responsive, with the navigation collapsing on narrower screens. This was a manual functional and responsiveness check rather than a per-page visual regression pass.
...
- The automated suite runs on Chromium only; the cross-browser and responsiveness checks were manual. Extending the Playwright projects to Firefox and WebKit would bring them into CI.
